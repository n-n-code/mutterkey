# Feature: Superseded Requires Pointer

## Motivation

Exercise lifecycle validation for superseded plans.

## Proposed Behavior

The checker should reject `superseded` plans that do not point to a replacement.
...
- Must remain true: Lifecycle state should communicate plan status clearly.
- Must become true: Superseded plans require a pointer to the replacement plan.
...
- Validation cost: low
- Notes: This fixture only checks lifecycle validation.
...
- Tests | impact=yes | status=passed | rationale=The checker behavior is under test. | verifier_note=The fixture should fail on lifecycle validation.
...
- Status: planned
- Notes: The lifecycle pointer is intentionally missing.
...
- Observed result: The checker should reject the missing replacement pointer.
