...
#include "transcription/cputokensearch.h"
...
std::optional<CpuDecodeResult> runCpuDecodePass(const CpuDecodeRequest &request)
{
    ...
    if (request.model->kind == CpuReferenceModelKind::FixtureV1) {
        ...
        const CpuGreedySearchConfig searchConfig{
            .sotTokenId = request.execution != nullptr ? request.execution->bosTokenId : 50257,
            .eotTokenId = request.execution != nullptr ? request.execution->eosTokenId : 50256,
        };
